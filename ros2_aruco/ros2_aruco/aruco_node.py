...
class ArucoNode(Node):
	def __init__(self):
		...
		self.calibration = np.array([[584.32262583, 0.0, 253.41645929], [0.0, 574.60800033, 233.93778173], [0.0, 0.0, 1.0]])
		self.distortion = np.array([[0.33706852, -1.65778338, -0.00908046, -0.04888188, 3.89528563]])
	...
	def image_callback(self, img_msg):
		...
		if ids is not None:
			...
			for i, id in enumerate(ids):
				...
				(row, pitch, yaw) = euler_from_rvec(cv2.Rodrigues(rvecs[i])[0])
			...
			self.get_logger().info('Publishing: "%s"' % ('Pose - x: {} y: {} z: {}').format(pose.position.x, pose.position.y, pose.position.z))
